...
(function() {
  ...
  function safeGetItem(key) {
    try {
      ...
      console.warn('[ShopifyLiveShipping] localStorage not available');
      return null;
    }
  }
  ...
  const ZIP_TO_STATE = {
    ...
    '911': 'CA', '912': 'CA', '913': 'CA', '914': 'CA', '915': 'TX', // El Paso TX
    '916': 'CA', '917': 'CA', '918': 'CA', '919': 'CA', '920': 'CA',
    '921': 'CA', '922': 'CA', '923': 'CA', '924': 'CA', '925': 'CA',
    '926': 'CA', '927': 'CA', '928': 'AZ', // Arizona exception
  ...
  function getStateFromZip(zip) {
    if (!zip) return 'NJ';
    const prefix = String(zip).padStart(5, '0').substring(0, 3);
  ...
  class ShopifyLiveShipping {
    ...
    async prepareRates(address) {
      ...
      try {
        ...
        if (!response.ok) {
          this.log('Prepare failed:', response.status, response.statusText);
        ...
        clearTimeout(timeoutId);
        this.log('Prepare error:', error.message);
    ...
    async getShippingRates(address) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          this.log('Fetch failed:', response.status, errorText);
          return { success: false, error: `HTTP ${response.status}`, rates: [] };
        }
        
        const data = await response.json();
        this.log('Raw rates:', data);
        ...
        if (rates.length > 0) {
          ...
          const fastest = rates.reduce((min, r) => 
            (r.delivery?.minDays || 99) < (min.delivery?.minDays || 99) ? r : min
          , rates[0]);
        ...
        return {
          ...
          rates,
          cheapest: rates[0],
          fastest: rates.find(r => r.isFastest),
          express: rates.filter(r => r.delivery?.minDays <= 2)
        };
      } catch (error) {
        clearTimeout(timeoutId);
    ...
    estimateDeliveryDays(name) {
      ...
      if (n.includes('next day') || n.includes('overnight') || n.includes('1-day') || n.includes('1 day')) return 1;
      if (n.includes('2 day') || n.includes('2-day') || n.includes('2nd day') || n.includes('two day')) return 2;
      if (n.includes('3 day') || n.includes('3-day') || n.includes('express') || n.includes('saver')) return 3;
    ...
    extractCarrier(name) {
      ...
      if (n.includes('fedex')) return { name: 'FedEx', icon: 'fedex' };
      if (n.includes('usps')) return { name: 'USPS', icon: 'usps' };
      if (n.includes('dhl')) return { name: 'DHL', icon: 'dhl' };
  ...
  console.log('[ShopifyLiveShipping] v2.0.0 loaded - Province fix applied');
})();
