...
async function getR2UploadUrl(
  ...
  try {
    ...
    // If a custom public domain is configured, use it (assumes public bucket or worker)
    if (config.r2PublicUrl) {
      publicUrl = `${config.r2PublicUrl}/${key}`
    } else {
      // If no public domain, assumes PRIVATE bucket.
      // Use application proxy to generate signed URLs on demand.
      let host = process.env.SHOPIFY_APP_URL || process.env.HOST || 'https://customizerapp.dev'
      if (!host.startsWith('http://') && !host.startsWith('https://')) {
        host = `https://${host}`
      }
      
      // Generate a long-lived token for the proxy URL (1 year)
      // The actual R2 signed URL will be generated by the proxy when accessed
      const expiresAt = Date.now() + 365 * 24 * 3600 * 1000
      const token = generateLocalFileToken(`r2:${key}`, expiresAt)
      publicUrl = `${host}/api/files/r2:${encodeURIComponent(key)}?token=${token}`
    }
