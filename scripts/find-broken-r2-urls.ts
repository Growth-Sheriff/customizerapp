...
const SHOP_DOMAIN = process.env.SHOPIFY_SHOP_DOMAIN || 'customizerapp.myshopify.com' // Fallback or logic to get it
const API_VERSION = '2025-01'
...
async function fetchOrders(cursor = null) {
  ...
  // Log status to debug potential 401s or 404s
  if (!response.ok) {
...
async function main() {
  console.log('🔍 Scanning orders for broken R2 URLs (pub-*.r2.dev)...')
  ...
  while (hasNextPage && processedCount < maxLimit) {
    ...
    process.stdout.write(`\rChecked ${processedCount} orders... Found ${brokenOrders.length} broken.`)
  }
  ...
  let markdown = '# 🚨 Broken R2 URLs Report\n\n'
  markdown += `**Date:** ${new Date().toLocaleString()}\n`
  markdown += `**Total Orders Scanned:** ${processedCount}\n`
  markdown += `**Broken Orders Found:** ${brokenOrders.length}\n\n`
  
  markdown += '| Order | Date | Item | Property | Broken URL |\n'
  markdown += '|-------|------|------|----------|------------|\n'

  for (const order of brokenOrders) {
    for (const item of order.items) {
      markdown += `| ${order.orderNumber} | ${order.date} | ${item.title} | ${item.key} | \`${item.value.substring(0, 40)}...\` |\n`
    }
  }

  // Also include a section for IDs (hidden) or raw data for the update script
  markdown += '\n<!-- RAW DATA FOR FIXING SCRIPT\n'
  markdown += JSON.stringify(brokenOrders, null, 2)
  markdown += '\n-->\n'
